...
/*                   グローバル変数宣言                    */
/***********************************************************/
unsigned char imageFace[MAX_IMAGESIZE][MAX_IMAGESIZE]; //入力画像用配列
int x_face_size, y_face_size;                               //image1の横画素数，縦画素数


/***********************************************************/
/*                   関数プロトタイプ宣言                  */
/***********************************************************/
void load_face_image_data(); //画像読み込み用関数
...
void load_face_image_data(){
	...
	/***** x_size1, y_size1 の代入 *****/
	x_face_size = 0; //x_size1の初期化
	y_face_size = 0; //y_size1の初期化
	while(x_face_size == 0 || y_face_size == 0){

		fgets(buffer,MAX_BUFFERSIZE,fp);
		if(buffer[0] != '#')//#から始まるコメントは読み飛ばす
			sscanf(buffer,"%d %d",&x_face_size,&y_face_size);
	...
	/***** 読込む画像が適切かチェック *****/
	if(x_face_size > MAX_IMAGESIZE || y_face_size > MAX_IMAGESIZE){//画像の大きさが想定値以内か調べる
	...
	for(y = 0;y < y_face_size;y++)
		for( x= 0;x < x_face_size;x++)
			imageFace[y][x] = (unsigned char)fgetc(fp);

	printf("顔画像のデータは正しく読み込まれました\n");
	printf("------------------------------------------------------------\n");
	fclose(fp);
